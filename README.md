...
-Create a command ( ./vsdsynth) which helps to transfer the .csv file from the UNIX shell to TCL script.
-Convert to format[1] and SDC format. Format[1] is readable by yosys tool.
-Convert to format[2] and .timing format to pass to Opentimer tool.
-Outputs a performance chart.
...
The algorithm to obtain the .sdc format from the constraints.csv file is as follows: Based on the clock, input or output details, the search space is figured out. The period is appended and duty cycle is calculated for each clock port. For, input ports the ports have to be identified as single-bit or multi-bit (busses) ports. A similar approach is used to get the respective details of the output ports.
...
Hierarchy check is performed to check whether all modules are well connected in all the *.v files. To check for an error, match the pattern "referenced in module" with very line of the hierarchy.log file. If a matching pattern is found, output the error message along with the path to its source file.
...
To generate a script for OpenTimer, procs are used in this project. Procs are external Tcl files that define reusable operations, similar to functions in Python. When these proc files are sourced into the main Tcl script, their commands can be called with specific arguments.
